...
import { getEcoGrowStageByLegacy } from "@/data/ecogrow-stages";
import { cn } from "@/lib/utils";
import type { MissionStageProgress, PancanitiStage } from "@/types/ecogrow";

const stageCopy: Record<PancanitiStage, { label: string; hint: string }> = {
...
export function PancanitiStepper({ stages, activeStage, onSelect, compact = false }: PancanitiStepperProps) {
  return (
    <div className="overflow-x-auto pb-1">
      <div className={cn("grid min-w-[760px] gap-3", compact ? "md:grid-cols-5" : "md:grid-cols-5")}>
        {stages.map((item, index) => {
          const stage = getEcoGrowStageByLegacy(item.stage);
